...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        backToTopButton.className = 'fixed bottom-6 right-6 bg-blue-600 text-white p-3 rounded-full shadow-lg hover:bg-blue-700 transition-all duration-200 opacity-0 invisible z-40';
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Escape') {
                const modalOpen = Alpine.$data(document.querySelector('[x-data]')).modalOpen;
                if (modalOpen) {
                    Alpine.$data(document.querySelector('[x-data]')).closeModal();
                }
            }
        });
    });
